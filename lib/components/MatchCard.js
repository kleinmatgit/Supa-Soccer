...
class MatchCard extends React.Component {
  ...
  getClassNameDate = (status) => {
    let cls = '';
    if(status === 'FT') {
      cls += ' datetime-match-finished';
    }
    return cls;
  }
  ...
  render() {
    ...
    let status = StatusHelper.getStatusDisplayName(this.props.match.status);
    let statusClsName = StatusHelper.getStatusClassName(this.props.match.status);
                ...
                <td>
                  <div className="match-card-status-table">
                    <table>
                      <tbody>
                        <tr>
                          <td className={statusClsName}>{status}</td>
                        </tr>
                        <tr>
                          <td className={this.getClassNameDate(status)}>{this.formatDate()}</td>
                        </tr>
                        <tr>
                          <td className={this.getClassNameDate(status)}>{this.formatTime()}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
